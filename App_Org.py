...
@st.cache_resource
def init_r2_cors():
    """Configure CORS sur le bucket R2 pour autoriser les PUT depuis le browser."""
    try:
        get_r2().put_bucket_cors(
        ...
        pass  # Ignorer si déjà configuré ou pas les permissions
...
class DataStore:
    ...
    def presigned_put_json(self, fid: str) -> str:
        """URL presignée PUT pour sauvegarder le JSON live de la map."""
        try:
            return get_r2().generate_presigned_url(
                "put_object",
                Params={"Bucket": _bkt(), "Key": f"maps/{fid}_live.json",
...
def init_state():
    init_r2_cors()  # Configurer CORS R2 une seule fois
    defs = {
...
function saveNow(force){{
  ...
  _saveTimer=setTimeout(()=>{{
    ...
    const rows=objs.map(o=>{{
      let c={{}};
      if(o.type==='r')c={{x:o.x,y:o.y,w:o.w,h:o.h}};
      else c={{x1:o.x1,y1:o.y1,x2:o.x2,y2:o.y2,srcId:o.srcId,dstId:o.dstId}};
      return{{object_id:o.id,type:o.type==='r'?'rectangle':'arrow',
              label:o.label||'',coords:JSON.stringify(c),writable:o.w2?'false':'true'}};
    }});
    const payload=JSON.stringify({{fid:FID,loc:LOC,rows,ts:Date.now()}});
    if(!PUT_URL){{stEl.textContent='⚠';setTimeout(()=>stEl.textContent='',2000);return;}}
    fetch(PUT_URL,{{
      method:'PUT',body:payload,
      headers:{{'Content-Type':'application/json'}}
    }}).then(r=>{{
      stEl.textContent=r.ok?'✓':'⚠ '+r.status;
      setTimeout(()=>stEl.textContent='',2000);
    }}).catch(()=>{{stEl.textContent='⚠';setTimeout(()=>stEl.textContent='',2000);}});
  }},600);
}}
...
def render_map():
    ...
    # Lire le JSON live si présent (sauvegardé par le canvas JS)
    try:
        obj = get_r2().get_object(Bucket=_bkt(), Key=f"maps/{fid}_live.json")
        raw = obj["Body"].read().decode("utf-8")
        payload = json.loads(raw)
        if payload.get("fid") == fid:
            _save_map_from_payload_rows(ds, fid, payload)
    except: pass
    ...
    # Générer presigned URL PUT pour sauvegarde directe JS→R2
    put_url = ds.presigned_put_json(fid)
    ...
    st.markdown(
        ...
        '💡 Alt+Drag: pan | Molette: zoom | Ctrl+Z/Y: undo/redo | ' +
        '💾 Auto: sauvegarde automatique | 💾 Sauv.: sauvegarde immédiate</div>',
        unsafe_allow_html=True)


def _save_map_from_payload(ds: DataStore, fid: str, loc: str, payload: dict):
    _save_map_from_payload_rows(ds, fid, payload, loc=loc)

def _save_map_from_payload_rows(ds: DataStore, fid: str, payload: dict, loc: str = ""):
    ...
    if not loc:
        try:
            ...
            if df_ex is not None:
                meta = df_ex[df_ex["type"] == "meta"]
                if not meta.empty: loc = str(meta.iloc[0]["_location_"])
        except: pass
    rows_out = [{"_location_": loc, "object_id": "_meta_",
                 "type": "meta", "label": "map", "coords": "{}", "writable": "false"}]
    for r in rows_in:
        t = r.get("type", "")
        if not t: continue
        rows_out.append({"_location_": "", "object_id": str(r.get("object_id", "")),
                         "type": t, "label": str(r.get("label", "")),
